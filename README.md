...
## Discussion
missing elaboration on aspects as pre-processing. Furthermore, because the source code was broken and due to budget and time constraints, we could not check if the exact same results could be achieved when using their code to train from scratch. Additionally, in this blog, the results are shared of our own implementation of their model. It may not be an exact match of the model of the authors, but it is still quite similar. However, the aspect of the pre-processing filter did not exactly match the performance in the paper. Despite these facts, it is shown that we came relatively close to the mIoU accuracies described in the paper[[5]](#5).  
